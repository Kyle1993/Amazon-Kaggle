...
def train_3epoch(model_path,lr,train_batch_size,validate_batch_size,validate_batch_num,resize,train_gpu,validate_gpu=-1):
    ...
    for fold in range(k):
        ...
        # model = AM_densenet161()
        # model = AM_resnet152()
        # model = AM_squeezenet11()
        # model = AM_vgg13()
        # model = AM_densenet121()
        # model = AM_resnet50()
        # model = AM_densenet161()
        # model = AM_resnet18()
        # model = AM_densenet201_new()
        # model  = AM_resnet34_new()
        # model = AM_resnet50_new()
        model = AM_resnet101_new()
        ...
        for epoch in range(epochs):
            ...
            if epoch==0:
                ...
                optimizer = torch.optim.Adam(model.parameters(), lr=lr/500, weight_decay=weight_decay)

            dset_train = AmazonDateset_train(train_index, IMG_TRAIN_PATH, IMG_EXT, LABEL_PATH, resize=resize)
            train_loader = DataLoader(dset_train, batch_size=train_batch_size, shuffle=True, num_workers=3)
            model.train()
